...
class GhazalsTab extends StatefulWidget {
  const GhazalsTab({
    super.key,
    required this.id,
  ...
  @override
  State<GhazalsTab> createState() => _GhazalsTabState();
}

class _GhazalsTabState extends State<GhazalsTab> {
  ...
  Future<void> putGhazal() async {
    final response =
        await http.put(Uri.parse('http://192.168.18.185:8080/ghazals'),
    ...
    if (response.statusCode == 201) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Ghazal Updated')),
        );
      }
    } else {
  ...
  Future<void> deleteGhazal() async {
    final response = await http.delete(
      Uri.parse('http://192.168.18.185:8080/ghazals/${widget.id}'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
    );
    if (response.statusCode == 200) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Ghazal Deleted')),
        );
      }
    } else {
      throw Exception('Failed to post ghazal');
