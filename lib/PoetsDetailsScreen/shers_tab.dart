...
class ShersTab extends StatefulWidget {
  const ShersTab({
    super.key,
    required this.id,
  ...
  @override
  State<ShersTab> createState() => _ShersTabState();
}

class _ShersTabState extends State<ShersTab> {
  ...
  Future<void> putSher() async {
    final response =
        await http.put(Uri.parse('http://192.168.18.185:8080/shers'),
    ...
    if (response.statusCode == 201) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Sher Updated')),
        );
      }
    } else {
  ...
  Future<void> deleteSher() async {
    final response = await http.delete(
      Uri.parse('http://192.168.18.185:8080/shers/${widget.id}'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
    );
    if (response.statusCode == 200) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Sher Deleted')),
        );
      }
    } else {
      throw Exception('Failed to post sher');
